...
관리자 전용 기능은 캐릭터 기본 데이터 또는 모든 사용자가 참고하는 관리자 목표 기본값을 바꾸는 동작입니다. 관리자 목표 기본값은 런타임 파일이나 로컬 캐시가 아니라 Firestore `goal/data` 문서에서 공개 조회하고, 저장은 관리자 권한에서만 수행합니다.
...
| 관리자 목표 스테이터스 기본값 편집 | 파밍 카드의 `[data-toggle-admin-goals]` 편집 버튼과 `[data-goal-field]`, 목표 스탯/에코 관련 입력 UI | `app.js`의 `[data-toggle-admin-goals]` 클릭 핸들러가 `adminGoalEditing`을 토글하고, 목표 입력 핸들러들이 `updateGoal()` 및 관련 목표 수정 함수로 관리자 목표를 수정 | `goal/data.defaults`, `character.goals.admin`, `adminGoalDefaults` |
...
| 백업/불러오기 | `#exportButton`, `#importInput` | 백업 버튼은 `getPortableState()`를 JSON으로 내보내고, 불러오기는 JSON을 개인 상태로 병합/적용 | 개인 기록 JSON |
...
- `goal/data.defaults`, `character.goals.admin`, `adminGoalDefaults`를 바꾸는 모든 진입점은 관리자 전용입니다.
...
- 공개 목표 데이터는 `loadCloudGoalData()`가 Firestore `goal/data`를 단건 조회해 적용합니다.
- signed-out, signed-in, 관리자 흐름 모두 같은 `goal/data` 문서를 사용하며, 페이지/탭 전환마다 다시 조회하지 않습니다.
- `loadCloudAdminData()`는 로그인 흐름에서 관리자 캐릭터 데이터(`admin/characters`)만 조회해 공개 목표 로딩과 분리합니다.
- Firestore Rules는 `goal/{docId}` 읽기를 공개하고, 생성/수정/삭제는 `isAdmin()` 통과 시에만 허용합니다.
